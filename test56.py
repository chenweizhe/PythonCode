...
'''
 * @Author: PythonZhe 
 * @Date: 2018-03-16 09:14:36 
 * @Last Modified by:   PythonZhe 
 * @Last Modified time: 2018-03-16 09:14:36 
 * @Desc: 进程线程---Pool
'''
...
#     p = Pool(4) #同时跑4个进程,但是总的有5个进程的进程池
#     for i in range(5):
...
而task 4要等待前面某个task完成后才执行，这是因为Pool的默认大小在我的电脑上是4，因此，最多同时执行4个进程。这是Pool有意设计的限制，并不是操作系统的限制'''
# Pool 默认大小是CPU的核数
...
r = subprocess.call(['nslookup','www.python.org']) #启动一个执行命令的进程
print('Exit code',r)
...
# Queue:在父进程中创建两个子进程,一个往Queue里写数据,一个从Queue里读数据
